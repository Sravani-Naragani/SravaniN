...
<body>
<jsp:useBean id="dao" class="com.infinite.student.SubjectsDAO" />
<form action="FeedBackCount.jsp" style="text-align: center" name="fbform">

  <c:if test="${param.instructor == null }">
  Instructor:
  <select name="instructor" id="instructor" onchange="submit()">
    <option>Select Instructor</option>
    <c:forEach var="i" items="${dao.getInstructors()}">
    <option value="${i}">${i}
    </option>
    </c:forEach>
  </select><br/><br/>
  ...
  <c:if test="${param.instructor != null && param.subject != null }">
    <br/><br/><table border="3" align="center">
     ...
     <c:forEach var="fb" items="${dao.FbCount(inst,subj)}">
     <tr>
       <td>${fb[0]}</td>
       <td>${fb[1]}</td>
     </tr>
